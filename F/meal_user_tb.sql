...
CREATE TABLE `user_tb` (
  ...
  `u_sex` varchar(3) NOT NULL DEFAULT 'M',
  `u_birth` datetime NOT NULL,
  `u_mile` int unsigned DEFAULT '0',
  `u_rank` varchar(100) DEFAULT 'bronze',
  `u_count` int DEFAULT '0',
  `u_creDate` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `u_del` varchar(3) DEFAULT 'N',
  `u_delDate` datetime DEFAULT NULL,
  `u_lastLog` datetime DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`u_id`)
