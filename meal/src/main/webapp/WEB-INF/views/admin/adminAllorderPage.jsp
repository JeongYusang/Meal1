...
<form action="url" method="post" enctype="multipart/form-data">
	<div class="div1">
		<div class="div3">
			<div class="div3-1" >${adminInfo.a_name }
		</div> 
		</div>
		<div class="div4">
			<div class="tabmenu">
				...
				<div class="table-container">
					<table id="stable-striped">
 						<thead id="tap-head">
 							<tr id="top-table">
								<th onclick="sortTable(0)" class="header" width="98px">상품ID</th>
								<th onclick="sortTable(1)" class="header" width="98px">상품명</th>
								<th onclick="sortTable(2)" class="header" width="98px">카테고리</th>
								<th onclick="sortTable(3)" class="header" width="98px">수량</th>
								<th onclick="sortTable(4)" class="header" width="98px">가격</th>
								<th onclick="sortTable(5)" class="header" width="98px">등록상태</th>
								<th onclick="sortTable(6)" class="header" width="98px">할인</th>
 								<th onclick="sortTable(7)" class="header" width="120px">할인기간</th>
								<th onclick="sortTable(8)" class="header" width="98px">인분수</th>
						...
						<c:choose>
							<c:when test="${empty goodsList}">
								<tr>
									<td colspan=12 class="fixed"><strong>등록된 상품이 없습니다.</strong></td>
								</tr>
							</c:when>
						...
						<div class="" id="pagination">
							<c:forEach var="page" begin="1" end="10" step="1">
								<c:if test="${section >0 && page==1 }">
									<a
										href="${contextPath}/admin/AllorderPage.do?section=${section}-1&pageNum=${(section-1)*10+1 }">preview</a>
								</c:if>
								<a
									href="${contextPath}/admin/AllorderPage.do?section=${section}&pageNum=${page}">${(section)*10 +page}
								</a>
								<c:if test="${page == 10 }">
									<a
										href="${contextPath}/admin/AllorderPage.do?section=${section}+1&pageNum=${section*10}+1">다음</a>
								</c:if>
							</c:forEach>
						</div>
					</center>
								...
								<div class="main-container">
									<div class="table-container">
										<table id="stable-striped">
											<thead id="tap-head">
												<tr id="top-table">
													<th onclick="sortTable(0)" class="header" width="154px">주문번호</th>
													<th onclick="sortTable(1)" class="header" width="154px">상품명</th>
													<th onclick="sortTable(2)" class="header" width="154px">가격</th>
													<th onclick="sortTable(3)" class="header" width="154px">수량</th>
													<th onclick="sortTable(4)" class="header" width="154px">구매자</th>
													<th onclick="sortTable(5)" class="header" width="154px">배송주소</th>
													<th onclick="sortTable(6)" class="header" width="154px">주문상태</th>
												</tr>
											</thead>
											<c:choose>
