...
function readURL(input) {
	...
	function backToList(obj) {
		obj.action = "${contextPath}/board1/selectBoard1List.do";
		obj.submit();
	}
</script>
<title>새글 쓰기 창</title>
...
function result(){
	...
	<div class='board-wrap'>
		...
		<form name="frmArticle" method="post"
			action="${contextPath}/board1/board1Update.do"
			...
			<div class='table-wrap'>
				<table>
						...
						<c:when test="${not empty memberInfo }">
							<tr>
								...
								<td class="td3"><input class="B_Uid" type=text
									...
									<input type="hidden" value="user" name="reg_id"> <c:choose>
										<c:when test="${not empty board1VO.secret }">
											<div class="secret-wrap">
												<input class="B_Uid1" type="checkbox" value="Y"
													name="secret" checked>비밀글입니다.
											</div>
										</c:when>
										<c:otherwise>
											<div class="secret-wrap">
								...
								<td class="td3"><input class="B_Uid" type=text
									...
									<div class="secret-wrap">
										<input class="B_Uid1" type="checkbox" name="secret" value="Y"
											onclick="#">비밀글입니다.
									</div> <input type="hidden" value="${board1VO.parentNo }"
									name="parentNo"></td>
							</tr>
						</c:when>
					</c:choose>
					<tr>
						<th class="td1">제목</th>
						<td class="td2"><input type="text" value="${board1VO.title }"
							name="title" id="i_title" disabled /> <input type="hidden"
							value="${board1VO.title }" name="title"> <input
							type="hidden" value="${board1VO.parentNo }" name="parentNo">
							<input type="hidden" value="${board1VO.b_1_id }" name="b_1_id">
						</td>
					</tr>
					<tr>
						<th class="td1">내용</th>
						<td class="td2"><textarea rows="20" cols="70"
								value="${board1VO.content }" name="content" id="i_content"> ${board1VO.content } </textarea></td>
					</tr>
					<tr height="200px">
