	...
	<div id="main-wrap">
		<div class="container">
            ...
            <div id="text">
               ...
               <h4>---------------------------------------------</h4>
               <h2>할인기간 : <fmt:formatDate value="${goodsInfo.g_saleDate1}" type="Date" pattern="yy년 MM월 dd일"/> ~ 
						   <fmt:formatDate value="${goodsInfo.g_saleDate2}" type="Date" pattern="yy년 MM월 dd일"/></h2>
            </div>
				<div id="price">
							...
							<c:choose>
								<c:when test="${empty boardGrList}">
									<tr class="border-bottom asdf">
										<td colspan=5 class="fixed"><strong>등록된 상품이
												없습니다.</strong></td>
									</tr>
								</c:when>
										...
										<div id="box11">
											...
											<ul id="slider1">
												<li id="slider1-1"><span id="slide1-11">${item.title}
													...
													<p>
														<img
															src="${contextPath}/thumbnailsBoard.do?b_gr_id=${item.b_gr_id}&fileName=${item.fileName }" /><br>
														${item.content}
													</p>
									...
									<c:forEach var="item" items="${boardGqList}" begin="0" end="10">
										<ul id="slider1">
											<li id="slider1-1"><span id="slide1-11">${item.title}
													${item.u_id} </span>
												<p CLASS="F"><img
															src="${contextPath}/thumbnailsBoardGq.do?b_gq_id=${item.b_gq_id}&fileName=${item.fileName }" /><br>
														${item.content}
													</p>
												<p class="pDDAK">${item.creDate}</p></li>
										</ul>
